...
def create_app(config_name):
    ...
    def monitoramento():
        if ( request.method == "GET" ):
            ...
            query = conn.execute(text('select * from monitoramento order by dt_created DESC LIMIT 20'))
           ...
           conn.commit()
           query = conn.execute(text('select * from monitoramento order by id'))
           result = [dict(zip(tuple(query.keys()), i)) for i in query.cursor]
           return jsonify(result)
